...
import { createClient } from "@/utils/supabase/server"; // Pastikan path ini benar
import { redirect } from "next/navigation";
import Link from "next/link";

export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  const supabase = await createClient(); // Gunakan await untuk server client terbaru
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0a] text-white">
      ...
      <aside className="w-64 border-r border-white/10 p-6 flex flex-col bg-[#0d0d0d]">
        <div className="mb-10">
          <h2 className="text-accent font-mono text-[10px] tracking-[0.3em] uppercase mb-1">System Control</h2>
          <h1 className="text-xl font-black tracking-tighter">ZAINI_OS <span className="text-white/20">v1.0</span></h1>
        </div>
        ...
        {/* Logout Form menggunakan Server Action */}
      ...
      <main className="flex-1 overflow-y-auto bg-[#0a0a0a] relative">
        {/* Overlay Grid Effect (Optional biar makin estetik) */}

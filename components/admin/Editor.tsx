...
function ToolbarButton({ onClick, active, disabled, title, children }: ToolbarButtonProps) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`p-1.5 transition-all rounded-sm ${
        active
...
export default function Editor({ value, onChange }: EditorProps) {
  ...
  // Sync nilai dari database (penting saat mode Edit)
  useEffect(() => {
    if (!editor) return
  ...
  // Paste gambar langsung ke editor
  ...
  if (!editor) return <div className="h-[320px] bg-white/[0.02] border border-white/10 animate-pulse" />
